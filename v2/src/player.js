...
static liftCount = 0; // 1ターンの上昇回数をカウント
    ...
    static createNewPuyo() {
        ...
        this.puyoStatus = {
            ...
            centerPuyo: this.centerPuyo,
            movablePuyo: this.movablePuyo,
        };
        // 接地時間はゼロ
        this.groundFrame = 0;
        // ぷよを描画
    ...
    static falling(isDownPressed) {
        ...
        currentPuyoX = x;
        currentPuyoY = y;
        window.puyoStatus = this.puyoStatus;
...
if (cy === -1 && this.liftCount < 6) { // 6回まで上昇可能
    if (this.groundFrame > 0) {
        this.puyoStatus.y -= 1;
        this.groundFrame = 0;
        this.liftCount++; // 上昇回数をカウント
    }
...
static fix() {
    ...
    this.liftCount = 0; // ぷよが固定されたらリセット
