...
static checkErase(startFrame) {
    ...
    for (let y = 0; y < Config.stageRows; y++) {
        for (let x = 0; x < Config.stageCols; x++) {
            ...
            if (sequencePuyoInfoList.length === 0 || sequencePuyoInfoList.length < Config.erasePuyoCount) {
                ...
                for (const info of sequencePuyoInfoList) {
                    const adjacentCells = [
                        [info.x, info.y + 1],
                        [info.x + 1, info.y],
                        [info.x, info.y - 1],
                    ...
                    for (const [adjX, adjY] of adjacentCells) {
                        if (adjX >= 0 && adjY >= 0 && adjX < Config.stageCols && adjY < Config.stageRows) {
                            const adjacentCell = this.board[adjY][adjX];
                            if (adjacentCell && adjacentCell.puyo === 6) {  // グレーぷよ（色6）
    ...
    if (ratio > 1) {
        ...
        for (const info of this.erasingPuyoInfoList) {
            ...
            if (this.stageElement.contains(element)) {  // elementがstageElementの子ノードか確認
                this.stageElement.removeChild(element);  // 完全にDOMから削除
            }
            // ここでグレーぷよをboardからも削除
            if (info.cell.puyo === 6) {
        ...
        for (const info of this.erasingPuyoInfoList) {
            ...
            // グレーぷよの場合、完全に削除する
            if (info.cell.puyo === 6) {
                if (this.stageElement.contains(element)) {
                    this.stageElement.removeChild(element);  // グレーぷよはこのタイミングで削除
                }
                this.board[info.y][info.x] = null;  // boardからも削除
            }
        }
        return true;
    } else if (ratio > 0.5) {
