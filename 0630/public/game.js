...
function generateNextPiece() {
    ...
    nextPiece = createPiece();
    if (!isValidMove(currentPiece.blocks) || !isValidMoveGrayPuyos()) {
        gameOver = true;
...
function isValidMoveGrayPuyos() {
    // グレーぷよが置けるかどうかをチェックする
    for (let y = 0; y < ROWS; y++) {
        for (let x = 0; x < COLS; x++) {
            if (
                board[y][x] === "gray" &&
                !isValidMove([{ x, y, color: "gray" }])
            ) {
                return false;
            }
        }
